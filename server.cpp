...
std::string Server::handleDeleteRequest(const HTTPRequest& request, const Location& location) {
    std::string file_path = buildFilePath(request.getPath(), location);
...
std::string Server::handleCGIRequest(const HTTPRequest& request, const Location& location, const std::string& file_path) {
    ...
    */
    ...
    std::cout << "[DEBUG] handleCGIRequest - Final response length: " << http_response.length() << std::endl;
    std::cout << "[DEBUG] handleCGIRequest - Response headers end at: " << http_response.find("\r\n\r\n") << std::endl;
...
std::string Server::generateDirectoryListing(const std::string& dir_path) {
